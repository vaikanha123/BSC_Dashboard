...
- **Month landing (added 2026-09-26):** the month-end landing is MTD actual + a rest-of-month forecast chosen by `score_landing()`/`choose_landing()`. Default `avg_start_latest_noB` = average of the forecast made just before the month began and the newest forecast's methods A+G (B is noisy over a part-month, and the newest forecast over-reacts to a strong/weak start that usually doesn't persist). Backtest over 513 origins: landing miss 3.44% -> 3.20%, day 8-14 4.63% -> 3.87%; day-14 landing miss per month 4.2% -> 3.5%. Each run re-scores all candidates on the last 365 days of the log and switches only if one beats the default by 0.15pp (needs >= 60 scored origins). Regions/stores are scaled by the same factor so they add up to the network landing. The daily chart/next-7-days still use the plain daily forecast.
