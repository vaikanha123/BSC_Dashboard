...
## 6b. Forecast tab (`FORECAST`, `scripts/forecast.py`, `data/`)

Offline-store revenue forecast: current-month landing vs target, next 7 days, next month, per region/store, plus an accuracy section. Added 2026-09-25.
...
- **Daily:** `daily_refresh.py` runs `forecast.py run --sales <MTD csv> --html index.html` as its last step (~30 s). It merges the MTD file into the history (idempotent — replaces those dates), retrains on everything, forecasts, appends to the log and rewrites `const FORECAST`. Non-fatal: on failure `data/` is restored and the tab keeps the previous day's numbers. **Commit `data/` together with the HTML files.**
- **Month transition (manual):** the daily task skips the 1st/2nd, so the last day of the closed month never reaches the history through it. Run `forecast.py run --sales "<complete previous month csv>" --sales "<new MTD csv>" --html index.html` after the `--new-month` dashboard update. If you forget, the next run stops with a "gap" message rather than training on a hole.
...
- **"Why this forecast" section:** `compute_insights()` in `forecast.py` derives every explanation on the tab from the history on each run — the month-to-month bridge (month length, weekend-count calendar effect, learned season/festival/trend), same-store next÷this-month ratios for past years (weekend-adjusted) vs what the forecast assumes, per-region festival-window lift, 13-week same-store YoY growth, weekend multipliers, the three methods' separate totals, and store movers vs the same 4 weeks last year. The JS only turns these numbers into sentences — don't hard-code claims there; add a computed field instead. The collapsible "The science behind it" text is static except the backtest numbers — update it if the method changes.
